...
static int fm10k_update_vid(struct net_device *netdev, u16 vid, bool set)
{
	...
	netdev_for_each_uc_addr(ha, netdev)
		fm10k_uc_vlan_unsync(netdev, ha->addr);
...
static void fm10k_vlan_rx_kill_vid(struct net_device *netdev, u16 vid)
{
	/* update VLAN and address table based on changes */
	...
	if (xcast_mode != FM10K_XCAST_MODE_PROMISC) {
		...
		if (xcast_mode != FM10K_XCAST_MODE_ALLMULTI)
			netdev_for_each_mc_addr(mclist, dev)
				fm10k_mc_unsync(dev, mclist->da_addr);
	}
...
void fm10k_restore_rx_state(struct fm10k_intfc *interface)
{
	...
	if (xcast_mode != FM10K_XCAST_MODE_PROMISC) {
		...
		if (xcast_mode != FM10K_XCAST_MODE_ALLMULTI)
			netdev_for_each_mc_addr(mclist, netdev) {
				fm10k_mc_unsync(netdev, mclist->da_addr);
...
{
	struct net_device __always_unused *netdev = interface->netdev;
	struct fm10k_hw *hw = &interface->hw;
...
#if 0
	/* NOTHING TO DO FOR RHEL6 since the callback is NULL */
...
struct net_device *fm10k_alloc_netdev(void)
{
	...
	netdev_extended(dev)->hw_features |= dev->features;
